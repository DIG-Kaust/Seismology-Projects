...
class SegyShot:
    ...
    def get_cmp(self, cmp_idx = 240, half_cmp=False, comp = None , d=None):
        if d==None :
            with segyio.open(self.filename, "r", ignore_geometry=True) as sgy:
                d = segyio.collect(sgy.trace)
        
        if cmp_idx < 0:
            cmp_idx = np.abs(cmp_idx)
            idrec = np.arange(240-cmp_idx,240,1)
            idsrc = np.arange(239,-1 + (240-cmp_idx),-1)
        else:
            idrec = np.arange(0,cmp_idx,1)
            idsrc = np.arange(cmp_idx-1,-1,-1)
            
        cmp = {}
        for ic,component in enumerate(comp):
            dc = d[ic::self.ncomponents,:]
            dc = dc.reshape((240,240,2000))
            dc = dc[idrec,idsrc,:]
        ...
        offset = (self.recx_local[np.arange(cmp_idx-1,-1,-1)] - self.srcx_local[np.arange(0,cmp_idx,1)])/2
        if half_cmp ==True:
            if cmp_idx % 2 == 0 :
                offset = offset[cmp_idx//2-1:]
            else:
                offset = offset[cmp_idx//2:]
